...
function checkCards() {
    ...
    check_line.forEach(function(value, index) {
        setTimeout(
            function(){
                $.ajax({
                    url: apis + '?cc_info=' + value + '&sk=' + sk_live + '&referrer=akahiko&telebot=' + telebot + "&tele_msg=" + tele_msg,
                    type: 'GET',
                    async: true,
...
function checkSK(){
    ...
    setTimeout(
        function(){
            $.ajax({
            url: 'skcheck.php?sk=' + sk_live + '&referrer=akahiko',
            type: 'GET',
            async: true,
            beforeSend: function () {
...
function testBot(){
    ...
    setTimeout(
        function(){
            $.ajax({
            url: 'telebot.php?telebot=' + telebot + '&referrer=akahiko',
            type: 'GET',
            async: true,
...
function credits(){
    if(!$("#footer").length){
        $("#container").append('<div class="footer" id="footer"><center><p style="color: #FFFFFF">akahiko</p></center></div>');
    }else{
